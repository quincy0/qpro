...
func (p *Prometheus) sendMetricsToPushGateway(metrics []byte) {
	...
	if _, err = client.Do(req); err != nil {
		qLog.Error("Error sending to push gateway. ", zap.Error(err))
	}
}
...
func (p *Prometheus) registerMetrics(subsystem string) {
	for _, metricDef := range p.MetricsList {
		...
		if err := prometheus.Register(metric); err != nil {
			qLog.Error("could not be registered in Prometheus", zap.String("metricName", metricDef.Name))
		}
		switch metricDef {
		case reqCnt:
...
func (p *Prometheus) HandlerFunc() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		p.reqDur.WithLabelValues(status, url, serviceCode, c.Request.Host,
			qConfig.Settings.Application.Mode,
